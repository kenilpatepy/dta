        ...
        try {
            if (subtitlePath.isNotEmpty()) {
                ...
                if (subtitleFile.exists()) {
                    ...
                    if (format == null) {
                        ToastCenter.showOriginalToast("Unsupported external subtitle format")
                        return null
                    }
                    val subtitleObj = format.parseFile(subtitleFile)
                    if (subtitleObj.captions.size == 0) {
                        ToastCenter.showOriginalToast("External subtitle content is empty")
                        return null
                    }
                    return subtitleObj
                }
            ...
            e.printStackTrace()
            ToastCenter.showOriginalToast("Failed to parse external subtitle file")
        }
        return null
    }
}