...
class ScreenShotView(
    ...
    init {
        ...
        viewBinding.shotSaveBt.setOnClickListener {
            ...
            if (isSuccess) {
                ToastCenter.showOriginalToast("$dirType: Save screenshot successfully")
            } else {
                ToastCenter.showOriginalToast("Failed to save screenshot")
            }
            onSettingVisibilityChanged(false)
        }
    }
    ...
    private fun prepareScreenShot() {
        ...
        mGenerateImageJob = mAttachActivity.lifecycleScope.launch(Dispatchers.Main) {
            ...
            if (bitmap == null) {
                ToastCenter.showOriginalToast("Failed to get screenshot")
                onSettingVisibilityChanged(false)
                return@launch
            }
