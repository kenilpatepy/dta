...
class PlayerActivity : BaseActivity<PlayerViewModel, ActivityPlayerBinding>(),
    ...
    private fun checkPlayParams(source: BaseVideoSource?): Boolean {
        if (source == null || source.getVideoUrl().isEmpty()) {
            CommonDialog.Builder(this).run {
                content = "Failed to parse playback parameters"
                addPositive("Exit and try again") {
                    it.dismiss()
                    finish()
                }
                build()
    ...
    private fun initListener() {
        ...
        danmuViewModel.downloadDanmuLiveData.observe(this) {
            if (it == null) {
                videoController.showMessage("Failed to download barrage")
                return@observe
            }
            ...
            videoController.showMessage("The barrage is loaded successfully")
        }
    }
    ...
    private fun showPlayErrorDialog() {
        ...
        val tips = if (source is StorageVideoSource && isTorrentSource) {
            ...
            "Playing failed, the resource has expired or is temporarily inaccessible, please try to switch the resource $taskLog"
        } else {
            "Playback failed, please try to change the player settings, or switch to another playback core"
        }
        ...
        val builder = AlertDialog.Builder(this@PlayerActivity)
            ...
            .setCancelable(false)
            .setMessage(tips)
            .setNegativeButton("quit playing") { dialog, _ ->
                dialog.dismiss()
                this@PlayerActivity.finish()
            }

        if (isTorrentSource) {
            builder.setPositiveButton("player settings") { dialog, _ ->
                dialog.dismiss()
                ARouter.getInstance()
    ...
    private fun switchVideoSource(index: Int) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            if (targetSource == null) {
                ToastCenter.showOriginalToast("The playback resource does not exist")
                return@launch
            }
