...
class SearchMagnetFragment :
                        ...
                        itemBinding.apply {
                            ...
                            contentView.setOnClickListener {
                                ...
                                if (magnetHash.isEmpty()){
                                    ToastCenter.showError("Error, the magnet link is empty or cannot be parsed")
    ...
    private fun initObserver() {
        ...
        viewModel.domainErrorLiveData.observe(this) {
            ToastCenter.showError("Please complete the node information")
            showInputDomainDialog()
        }
    }
    ...
    private fun showActionDialog(data: MagnetData) {
        BottomActionDialog(requireActivity(), actionData) {
            ...
            if (data.Magnet.isNullOrEmpty()) {
                ToastCenter.showError("The magnetic link information is empty and cannot be copied")
                return@BottomActionDialog true
            }
            if (it.actionId == ManageMagnet.Copy && magnetHash.isEmpty()) {
                ToastCenter.showError("Error, the magnet link is empty or cannot be resolved")
                return@BottomActionDialog true
            }
            when (it.actionId) {
                ManageMagnet.Copy -> {
                    "magnet:?xt=urn:btih:$magnetHash".addToClipboard()
                    ToastCenter.showSuccess("The magnet link has been copied!")
                }
                ManageMagnet.CopyContent -> {
                    data.Magnet?.addToClipboard()
                    ToastCenter.showSuccess("The magnet link information has been copied!")
                }
            }
            return@BottomActionDialog true
        }.show()
    ...
    private enum class ManageMagnet(val title: String, val icon: Int) {
        Copy("copy magnet link", R.drawable.ic_magnet_copy),
        CopyContent("Copy full magnet link", R.drawable.ic_magnet_copy_content);
