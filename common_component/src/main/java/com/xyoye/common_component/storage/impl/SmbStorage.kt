...
class SmbStorage(library: MediaLibraryEntity) : AbstractStorage(library) {
    ...
    override suspend fun pathFile(path: String, isDirectory: Boolean): StorageFile? {
        ...
        return try {
            ...
            e.printStackTrace()
            showErrorToast("Failed to get file information", e)
            null
        }
    }
    ...
    private fun checkConnection(): Boolean {
        ...
        try {
            ...
            e.printStackTrace()
            showErrorToast("Failed to connect to SMB service", e)
            close()
        }
        return false
    }
    ...
    private fun getRootShares(): List<SmbStorageFile> {
        return try {
            ...
            e.printStackTrace()
            showErrorToast("Failed to get shared directory list", e)
            emptyList()
        }
    }
    ...
    private fun switchShareDisk(shareName: String, showToast: Boolean = false): Boolean {
        ...
        try {
            ...
            e.printStackTrace()
            if (showToast) {
                showErrorToast("Failed to switch shared directory", e)
            }
