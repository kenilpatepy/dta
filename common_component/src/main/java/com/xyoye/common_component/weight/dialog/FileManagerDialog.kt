...
class FileManagerDialog(
    ...
    override fun initView(binding: DialogFileManagerBinding) {
        ...
        setTitle(
            when (action) {
                FileManagerAction.ACTION_SELECT_DANMU -> "Select local bullet chat file"
                FileManagerAction.ACTION_SELECT_SUBTITLE -> "Select local subtitle file"
                FileManagerAction.ACTION_SELECT_VIDEO -> "Select Video File"
                FileManagerAction.ACTION_SELECT_DIRECTORY -> "Select Folder"
                FileManagerAction.ACTION_SELECT_TORRENT -> "Select torrent file"
            }
        )
    ...
    private fun initTag() {
        ...
        addRightAction(defaultDrawable, 8, "Open the directory this time").setOnClickListener {
            openTargetDirectory(defaultFolderPath!!)
        }
        ...
        if (AppConfig.isLastOpenFolderEnable() && lastOpenFolderPath?.isNotEmpty() == true) {
            ...
            addRightAction(lastOpenDrawable, 8, "last opened directory").setOnClickListener {
