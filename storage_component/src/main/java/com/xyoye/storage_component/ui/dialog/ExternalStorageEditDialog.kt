...
import com.xyoye.storage_component.databinding.DialogExternalStorageEditBinding
import com.xyoye.storage_component.ui.activities.storage_plus.StoragePlusActivity
import com.xyoye.storage_component.utils.launcher.DocumentTreeLauncher
...
class ExternalStorageEditDialog(
    ...
    override fun initView(binding: DialogExternalStorageEditBinding) {
        ...
        if (library == null) {
            setTitle("Add Device Repository")
        } else {
            setTitle("Edit Device Repository")
    ...
    private fun createLibrary(): MediaLibraryEntity? {
        if (mDocumentFile == null) {
            ToastCenter.showError("Please select the device repository folder")
            return null
        }
    ...
    private fun getDisplayName(storage: MediaLibraryEntity): String {
        ...
        val documentFile = DocumentFile.fromTreeUri(activity, uri) ?: return uri.lastPathSegment  ?: return "Unknown repository"
    ...
    private fun getDisplayName(documentFile: DocumentFile): String {
        ...
        if (treeId == "primary:") {
            return "internal storage device"
        }
    ...
    private fun onResult() = block@{ uri: Uri? ->
        ...
        if (takePersistableUriPermission(uri).not()) {
            ToastCenter.showError("Failed to get folder access")
            return@block
        }

        val documentFile = DocumentFile.fromTreeUri(activity, uri)
        if (documentFile == null) {
            ToastCenter.showError("can't access folder")
            return@block
        }
