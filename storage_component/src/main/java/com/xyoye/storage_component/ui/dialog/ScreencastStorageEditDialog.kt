...
class ScreencastStorageEditDialog(
    ...
    override fun initView(binding: DialogScreencastConnectBinding) {
        ...
        //禁止拖动关闭弹窗
        disableSheetDrag()
        setTitle(if (isEditStorage) "Edit projection information" else "Add projection device")
    ...
    private fun BaseViewHolderCreator<ItemScreencastReceiveDeviceBinding>.deviceItem() = { data: UDPDeviceBean ->
        ...
        itemBinding.itemLayout.setOnClickListener {
            if (data.needPassword) {
                inputPassword(data)
                ToastCenter.showToast("Please enter the connection password")
            } else {
                connect(data)
            }
        }
    ...
    private fun connect(ip: String, port: String, displayName: String, password: String) {
        if (ip. isEmpty()) {
            ToastCenter.showError("IP address cannot be empty")
            return
        }
        if (port. isEmpty()) {
            ToastCenter.showError("The port cannot be empty")
            return
        }
        val httpPort = port.toIntOrNull()
        if (httpPort == null) {
            ToastCenter.showError("Port error")
            return
        }
        if (password.isEmpty() && binding.passwordConnectTv.isSelected) {
            ToastCenter.showError("The password cannot be empty")
            return
        }
        var deviceName = displayName
        if (displayName. isEmpty()) {
            deviceName = "Unknown mirroring device"
        }
