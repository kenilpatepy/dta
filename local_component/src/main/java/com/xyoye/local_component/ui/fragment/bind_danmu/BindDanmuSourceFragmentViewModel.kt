...
class BindDanmuSourceFragmentViewModel : BaseViewModel() {
        ...
        httpRequest(viewModelScope) {
            ...
            api {
                ...
                if (danmuPath. isNullOrEmpty()) {
                    ToastCenter.showError("Failed to save bullet chat")
                } else {
                    ToastCenter.showSuccess("Save the barrage successfully!")
    ...
    fun downloadDanmu(
        ...
        if (danmuSources. size == 0) {
            ToastCenter.showWarning("Please select at least one barrage source")
            return
        }
